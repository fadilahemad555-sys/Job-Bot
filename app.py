# ============================================================
# منصة بريكولات - الملف الرئيسي الكامل (نسخة معدلة)
# ✅ زر Google واحد فقط
# ✅ إكمال ملف مبسط (الاسم، التخصص، المدينة)
# ✅ جلسة دائمة
# ✅ لا توجد أسرار في الكود (كلها من متغيرات البيئة)
# ============================================================
...
# الباسورد يُقرأ من متغير البيئة MAIL_PASSWORD
...
# السري يُقرأ من متغير البيئة GOOGLE_CLIENT_SECRET
...
def complete_profile():
    ...
    <div class="container" style="max-width:600px;margin-top:50px">
        ...
        <form method="POST">
            <div class="mb-3"><label>الاسم الكامل <span class="text-danger">*</span></label><input type="text" name="full_name" class="form-control" value="{{ current_user.full_name or '' }}" required></div>
            <div class="mb-3"><label>المدينة <span class="text-danger">*</span></label><input list="cities" name="district" class="form-control" required><datalist id="cities">{% for city in MOROCCAN_CITIES %}<option value="{{ city }}">{% endfor %}</datalist></div>
            <div class="mb-3"><label>التخصص <span class="text-danger">*</span></label><select name="specialty" id="specialty_select" class="form-select" onchange="toggleSpecialtyInput()" required><option value="">اختر تخصصك</option>{% for spec in SPECIALTIES %}<option value="{{ spec }}">{{ spec }}</option>{% endfor %}<option value="other">تخصص آخر</option></select><input type="text" name="other_specialty" id="other_specialty" class="form-control mt-2" style="display:none;" placeholder="اكتب تخصصك"></div>
            <button type="submit" class="btn btn-primary w-100">تم</button>
        </form>
        <a href="/logout" class="btn btn-link">تسجيل خروج</a>
    </div></body></html>
    ''', User=User, MOROCCAN_CITIES=MOROCCAN_CITIES, SPECIALTIES=SPECIALTIES)# ============================================================
# الصفحة الرئيسية (بزر Google واحد فقط)
# ============================================================
@app.route('/')
def index():
    ...
    <!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1">
        ...
        <style>
            ...
            .apk-button{position:fixed;top:20px;right:20px;background-color:#ff9800;color:white;padding:8px 12px;border-radius:30px;font-size:14px;font-weight:bold;text-decoration:none;z-index:1000;box-shadow:0 2px 5px rgba(0,0,0,0.2);transition:0.3s;}
            .apk-button:hover{background-color:#e68900;transform:scale(1.05);}
        </style>
    </head>
    <body>
        ...
        <div class="container mt-4">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div>
                    ...
                    {% else %}
                        <a href="{{ url_for('login_google') }}" class="btn btn-primary">تسجيل الدخول عبر Google</a>
                    {% endif %}
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="request-card">
                        ...
                        {% endif %}
                        <a href="/delete-request/{{ req.id }}" class="btn btn-sm btn-danger" onclick="return confirm('هل أنت متأكد؟')">حذف</a>
                    </div>
                </div>
                {% else %}
        ...
        <script>
        function openModal(src){ document.getElementById('modalImage').src=src; new bootstrap.Modal(document.getElementById('imageModal')).show(); }
        </script>
            ...
            <div class="col-md-4 mb-4">
                <div class="card h-100">
                    ...
                    <div class="card-body">
                        ...
                        <p><strong>التخصص:</strong> {{ item.artisan.specialty }}<br><strong>الحي:</strong> {{ item.artisan.district or 'غير محدد' }}{% if item.artisan.experience_years %}<br><strong>الخبرة:</strong> {{ item.artisan.experience_years }} سنة{% endif %}</p>
                        {% if item.num > 0 %}<p class="text-warning">{% for i in range(item.avg|int) %}⭐{% endfor %} {{ item.avg }} ({{ item.num }})</p>{% else %}<p class="text-muted">لا توجد تقييمات</p>{% endif %}
                    </div>
                </div>
            </div>
            {% endfor %}
...
# ================== الملف الشخصي الخاص (بكل خياراته الكاملة) ==================
...
def profile():
    ...
    <div class="container mt-5" style="max-width:800px;">
        ...
        <div class="collapse" id="editForm"><div class="card card-body"><form method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3"><label>التخصص</label><select name="specialty" class="form-select">{% for spec in SPECIALTIES %}<option value="{{ spec }}" {% if current_user.specialty == spec %}selected{% endif %}>{{ spec }}</option>{% endfor %}<option value="other" {% if current_user.specialty not in SPECIALTIES %}selected{% endif %}>تخصص آخر</option></select></div>
...
def search():
    ...
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>.stats-mini{position:fixed;bottom:10px;left:10px;background:rgba(0,0,0,0.7);color:#fff;padding:5px 10px;border-radius:20px;font-size:12px;z-index:9999;opacity:0.6;}</style></head>
    <body>
    <div class="stats-mini">👥 {{ User.query.count() }} | 🔨 {{ User.query.filter_by(user_type='artisan').count() }}</div>
    <div class="container mt-5">
        ...
        <div class="row">
            {% for req in requests %}
            <div class="col-md-6 mb-4"><div class="card request-card"><div class="card-body">
                <div class="client-info"><img src="{{ req.client.profile_image or '/uploads/placeholder.jpg' }}" class="client-img"><div><strong><a href="/user/{{ req.client.id }}">{{ req.client.full_name or req.client.username }}</a></strong><br><small>{{ req.district }} · {{ time_ago(req.created_at) }}</small></div></div>
                <h5>{{ req.title }}</h5><p>{{ req.description }}</p>
                {% if req.images %}<div class="request-images">{% for img in req.images.split(',') %}<img src="{{ img }}" onclick="openModal('{{ img }}')">{% endfor %}</div>{% endif %}
                <div class="d-flex justify-content-between align-items-center mt-3"><span class="badge bg-primary">{{ req.specialty }}</span><span class="badge bg-info">عروض: {{ req.offers_count }}/30</span></div>
                <div class="mt-2"><a href="/user/{{ req.client.id }}" class="btn btn-sm btn-outline-secondary">عرض الملف</a>{% if current_user.is_authenticated and req.status == 'open' and req.offers_count < 30 %}<a href="/send-offer/{{ req.id }}" class="btn btn-sm btn-primary">تقديم عرض</a>{% endif %}</div>
            </div></div></div>
            {% endfor %}
        </div>
    </div>
...
def post_request():
    if request.method == 'POST':
        ...
        try:
            ...
            for user in all_users_with_email:
                try:
                    msg = MailMessage(subject=f"🔔 طلب جديد في تخصص {specialty} - {district}", recipients=[user.email], html=f"<h2>طلب جديد</h2><p>{description}</p><a href='https://bricoletsapp.pythonanywhere.com/'>عرض</a>")
                    mail.send(msg)
                    sent_count += 1
                except Exception as e:
                    print(e)
            if sent_count > 0:
                flash(f'✅ تم إرسال إشعارات إلى {sent_count} مستخدم', 'success')
        except Exception as e:
            print(e)
        return redirect(url_for('index'))
    return render_template_string('''
    ...
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>.stats-mini{position:fixed;bottom:10px;left:10px;background:rgba(0,0,0,0.7);color:#fff;padding:5px 10px;border-radius:20px;font-size:12px;z-index:9999;opacity:0.6;}</style>
    <script>function toggleSpecialtyInput(){var select=document.getElementById('specialty_select');var otherInput=document.getElementById('other_specialty');if(select.value==='other'){otherInput.style.display='block';otherInput.required=true;}else{otherInput.style.display='none';otherInput.required=false;}}</script>
    </head>
    <body>
    <div class="stats-mini">👥 {{ User.query.count() }} | 🔨 {{ User.query.filter_by(user_type='artisan').count() }}</div>
    <div class="container" style="max-width:600px;margin-top:50px">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3"><label>التخصص الذي تحتاجه</label><select name="specialty" id="specialty_select" class="form-select" onchange="toggleSpecialtyInput()" required><option value="">اختر التخصص</option>{% for spec in SPECIALTIES %}<option value="{{ spec }}">{{ spec }}</option>{% endfor %}<option value="other">تخصص آخر</option></select><input type="text" name="other_specialty" id="other_specialty" class="form-control mt-2" style="display:none;" placeholder="اكتب التخصص"></div>
            <div class="mb-3"><label>المدينة أو الحي</label><input list="cities" name="district" class="form-control" required><datalist id="cities">{% for city in MOROCCAN_CITIES %}<option value="{{ city }}">{% endfor %}</datalist></div>
...
def view_offers(request_id):
    ...
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>.stats-mini{position:fixed;bottom:10px;left:10px;background:rgba(0,0,0,0.7);color:#fff;padding:5px 10px;border-radius:20px;font-size:12px;z-index:9999;opacity:0.6;}.request-images{display:flex;flex-wrap:wrap;gap:10px;margin:15px 0;}.request-images img{width:100px;height:100px;object-fit:cover;border-radius:5px;cursor:pointer;}</style>
    </head>
    <body>
    <div class="stats-mini">👥 {{ User.query.count() }} | 🔨 {{ User.query.filter_by(user_type='artisan').count() }}</div>
    <div class="container mt-5">
        ...
        <h3>العروض المقدمة</h3><div class="row">
            ...
            <div class="col-md-4 mb-3"><div class="card"><div class="card-body">
                ...
                <div class="quick-message-box mt-2"><form method="POST"><input type="hidden" name="artisan_id" value="{{ item.artisan.id }}"><div class="d-flex gap-2"><textarea name="quick_message" class="form-control" rows="1" placeholder="اكتب رسالتك..."></textarea><button type="submit" class="btn btn-primary btn-sm">💬</button></div></form></div>
                <a href="/start-chat/{{ req.id }}/{{ item.artisan.id }}" class="btn btn-primary btn-sm mt-2">فتح المحادثة</a>{% if can_rate %}<a href="/rate/{{ item.artisan.id }}/{{ req.id }}" class="btn btn-warning btn-sm mt-2">تقييم</a>{% endif %}
            </div></div></div>
            {% endfor %}
        </div>
...
def admin_dashboard():
    ...
    <div class="container mt-5">
        ...
        <div class="row"><div class="col-md-3"><div class="card text-white bg-primary mb-3"><div class="card-body"><h5 class="card-title">المستخدمين</h5><p class="card-text display-6">{{ total_users }}</p><p>زبائن: {{ total_clients }} | حرفيين: {{ total_artisans }}</p></div></div></div>
        ...
        <div class="col-md-3"><div class="card text-white bg-info mb-3"><div class="card-body"><h5 class="card-title">المحادثات</h5><p class="card-text display-6">{{ total_chats }}</p></div></div></div></div>
        <div class="card mb-4"><div class="card-header bg-dark text-white d-flex justify-content-between align-items-center"><span>جميع الحرفيين ({{ total_artisans }})</span><a href="/artisans" class="btn btn-sm btn-light">عرض في الموقع</a></div><div class="card-body"><div class="table-responsive"><table class="table table-sm table-bordered"><thead><th>#</th><th>الاسم</th><th>التخصص</th><th>المدينة</th><th>البريد الإلكتروني</th><th>رقم الهاتف</th><th>تاريخ التسجيل</th></thead><tbody>{% for a in all_artisans %}<tr><td>{{ a.id }}</td><td><a href="/user/{{ a.id }}">{{ a.full_name or a.username }}</a></td><td>{{ a.specialty }}</td><td>{{ a.district or '-' }}</td><td>{{ a.email }}</td><td>{{ a.phone or '-' }}</td><td>{{ a.created_at.strftime('%Y-%m-%d') }}</td></tr>{% endfor %}</tbody></table></div></div></div>
    ...
    </body></html>''', total_users=total_users, total_clients=total_clients, total_artisans=total_artisans, total_requests=total_requests, total_offers=total_offers, total_chats=total_chats, recent_users=recent_users, recent_requests=recent_requests, chat_data=chat_data, time_ago=time_ago, all_artisans=all_artisans)
